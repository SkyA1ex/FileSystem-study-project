...
public class FileSystem {
	...
	private BTree<String, FileInfo> files; // Содержит информацию о файлах,
											// хранящихся на диске
	...
	public void add(File file) {
		if (files.find(file.getName()) != null)
			throw new FileAlreadyExistException("File " + file.getName()
					+ " already " + "exist;");
		...
		if (address != 0)
			...
			throw new NotEnoughMemoryException(
					"Not enough memory for allocate file: " + file.getName()
							+ ";");
	}
	...
	public int sizeOf(String fileName) {
		...
		if (fi != null)
			...
			throw new FileNotFoundException(
					"File with this name does't exist: " + fileName + ";");
	}
	...
	public File find(String fileName) {
		FileInfo fi = files.find(fileName);
		if (fi != null)
			return new File(fileName, disk.read(fi.address, fi.length),
					fi.length);
		else
			return null;
	}
