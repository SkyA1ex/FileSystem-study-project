...
public class Disk {
	...
	private boolean[] isFree; // Хранит информацию о свободных кластерах в
								// памяти
	...
	public int getDiskSize() {
		return diskSize * clusterSize;
	}

	/*
	 * Выделяет память для файла, возвращает адрес, если память данного размера
	 * может быть выделена и ноль в противном случае
	 */
	...
	public byte[] read(int startIndex, int length) {
		...
		if (start + count > diskSize)
			throw new IndexOutOfBoundsException("Index out of disk memory: "
					+ "diskSize = " + String.valueOf(diskSize) + "; index = "
